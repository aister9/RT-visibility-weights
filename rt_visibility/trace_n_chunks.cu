...
extern "C" __constant__ RTVisParamsNChunks optixLaunchParams;
...
extern "C" __global__ void __raygen__program__() {
    ...
    {
        if (optixLaunchParams.segMinMaxDistance[idx].y <= payload.rayTmax) {
            optixLaunchParams.segMinMaxDistance[idx].y = payload.rayTmax;
            optixLaunchParams.segMinMaxIdx[idx].y = payload.lastHit;
        }
    }

    if (payload.firstHit > 0)
    {
        if (optixLaunchParams.segMinMaxDistance[idx].x > payload.rayTmin) {
            optixLaunchParams.segMinMaxDistance[idx].x = payload.rayTmin;
            optixLaunchParams.segMinMaxIdx[idx].x = payload.firstHit;
        }
    }
}
